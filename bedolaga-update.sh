...
Ключ скрыт спойлером — нажмите, чтобы показать:

🔑 <tg-spoiler>${PRIV}</tg-spoiler>

🔓 Публичный ключ: <code>${PUB}</code>

<b>Восстановление:</b> bedolaga-update → «5) Восстановление» → вставьте этот ключ.

🧨 Это сообщение самоуничтожится через 3 минуты." 180; then
        success "Отправлено в Telegram (ключ под спойлером, удалится через 3 минуты)." >&2
      else
